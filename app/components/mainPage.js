...
export default class MainPage extends Component {
     ...
     takePicture() {
        ...
        this.camera.capture({ metadata: options })
            .then((data) => {
                ...
                var dataForward={
                    path:data.path,
                    size:this.state.size._value,
                    top:this.state.panTop._value,
                    left:-(this.state.panLeft._value),
                    angle:-(this.state.angle._value)+'deg'
                }
                this.props.navigation.navigate('ImageView',dataForward);
            })
    ...
    render() {
        return (
            <View style={{ flex: 1 }}>
                <Camera type={'front'} ref={(cam) => { this.camera = cam; } } style={styles.cameraStyle} >
                    <Animated.Image   {...this._panResponder.panHandlers} source={require('../images/shades.png') }  style={{
                        top: this.state.panTop, left: this.state.panLeft, height: this.state.size, width: this.state.size, transform: [{
                            rotate: this.state.angle.interpolate({
                                inputRange: [-360, 360],
                                outputRange: ['-360deg', '360deg']
                            })
                        }]
                    }} onLayout={(event) => { } } collapsable={false} resizeMode='contain' />
                </Camera>
                <TouchableOpacity style={styles.pictureButtonTouch} onPress={this.takePicture.bind(this) }>
